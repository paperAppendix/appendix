...
int main(int argc, char ** argv) {
	if (argc<3) {
		fprintf(stderr, "usage: pagerank [path] [iterations] [memory in GB]\n");
		exit(-1);
	}
	...
	for (int iter=0;iter<iterations;iter++) {
		...
		graph.stream_edges<VertexId>(
			[&](Edge & e){
                float diff = delta[e.source]/degree[e.source];
                if (diff > 0.05)
				    write_add(&sum[e.target], diff); 
				return 0;
			}, nullptr, 0, 1,
			...
			[&](std::pair<VertexId,VertexId> target_vid_range){
				// sum.lock(target_vid_range.first, target_vid_range.second);
				// sum.load(target_vid_range.first, target_vid_range.second);
			},
			[&](std::pair<VertexId,VertexId> target_vid_range){
				// sum.unlock(target_vid_range.first, target_vid_range.second);
				// sum.save();
			}
		);
		printf("%.2f\n", get_time() - begin_time);
	...
	float max_pagerank = 0;
	VertexId max_pagerank_vid;
	for (VertexId i=0;i<graph.vertices;i++) {
		if (max_pagerank < pagerank[i]) {
			max_pagerank = pagerank[i];
			max_pagerank_vid = i;
		}
	}
	printf("pagerank[%d] = %.2f\n", max_pagerank_vid, max_pagerank);
}
